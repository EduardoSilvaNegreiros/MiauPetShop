...
<form action="index.php?menuop=atualizar-servico" method="POST" class="form">
    <div class="input-group">
        ...
        <div class="input-box">
            ...
            <input type="text" name="idServico" id="idServico" value="<?= mysqli_real_escape_string($conexao, $dados['idServico']) ?>" readonly>
        </div>

        <!-- Campo Nome do Serviço -->
        <div class="input-box">
            <label for="nomeServico">Nome do Serviço</label>
            <input type="text" name="nomeServico" id="nomeServico" value="<?= mysqli_real_escape_string($conexao, $dados['nomeServico']) ?>" required>
        </div>

        <!-- Campo Descrição do Serviço -->
        <div class="input-box">
            <label for="descricaoServico">Descrição</label>
            <textarea name="descricaoServico" id="descricaoServico" required><?= mysqli_real_escape_string($conexao, $dados['descricao']) ?></textarea>
        </div>

        <!-- Campo Preço do Serviço -->
        <div class="input-box">
            <label for="precoServico">Preço</label>
            <input type="text" name="precoServico" id="precoServico" value="<?= mysqli_real_escape_string($conexao, $dados['preco']) ?>" required>
        </div>

        <!-- Campo Duração do Serviço -->
        <div class="input-box">
            <label for="duracaoServico">Duração (em horas)</label>
            <input type="text" name="duracaoServico" id="duracaoServico" value="<?= mysqli_real_escape_string($conexao, $dados['duracao']) ?>" required>
        </div>
